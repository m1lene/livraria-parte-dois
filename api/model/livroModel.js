...
export const listLivros = async () => {
    console.log(dbConfig);
    const connection = await mysql.createConnection(dbConfig);
    const query = "SELECT * FROM Livro";
    const [results] = await connection.query(query);
    console.log('Chamada de metodo do model: listLivros');
    console.log(query);    
    console.log(results);
    return results;
};
...
export const getLivrosById = async(id) => {
    const connection = await mysql.createConnection(dbConfig);
    const query = "SELECT titulo FROM Livro WHERE id=?";
    const [results] = await connection.query(query,[id]);
    console.log('Chamada de metodo do model: getLivrosById');
    console.log(query);
    console.log(results);
    return results
};
